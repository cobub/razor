...
public class UmsAgent {
	...
	private static int defaultReportMode = 0;// 0 send at next time，defaultmode
												// 1 send at now
	 ...
	 * 
	 * @param type
	 *            errorInfo/activityInfo/eventInfo/clinetDataInfo
	 * @param info
	 * @param context
	 */
	...
	private static void postEventInfo(Context context, String event_id, String label, int acc) {
		try {
	...
	public static void saveEvent(UmsAgent umsAgent, Context context, String appkey, String event_id, String label,
			int acc) {
	...
	private static void updateOnlineConfigs(Context context) {
		...
		SharedPreferences preferences = context.getSharedPreferences(
				"ums_agent_online_setting_" + CommonUtil.getPackageName(context), 0);
		Editor editor = preferences.edit();

		if (CommonUtil.isNetworkAvailable(context)) {
			MyMessage message = NetworkUitlity.post(UmsConstants.preUrl + UmsConstants.onlineConfigUrl, appkeyJSON);
			try {
	...
	private static void saveSessionTime(Context context) {
		// TODO Auto-generated method stub
		SharedPreferences preferences2sessiontime = context.getSharedPreferences("UMS_session_ID_savetime",
	...
	private static void uploadAllLog(Context context) {
		...
		File file1 = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/mobclick_agent_cached_"
	...
	private static JSONObject getClientDataJSONObject(Context context) {
		...
		try {
			...
			LatitudeAndLongitude coordinates = CommonUtil
					.getLatitudeAndLongitude(context, UmsAgent.mUseLocationService);

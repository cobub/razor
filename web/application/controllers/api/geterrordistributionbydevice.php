<?php 
class geterrordistributionbydevice extends CI_Controller
{
	function __construct(){
		parent::__construct();
		$this->load->model('api/common','common');
		$this->load->model('api/geterrordistributionbydevicemodel','geterrordistributionbydevice');
	}
	function index(){
...
// 		";
		...
		if($verify['flag']<=0)
		{
			...
			$ret=$this->geterrordistributionbydevice->getdata($content->sessionkey,$content->productid,$content->startdate,$content->enddate,$content->erroridentifier);
		echo json_encode($ret);
	}
}

...
<script type="text/javascript">
    ...
    var regionSessionCategories = [];
    var regionSessionPercentCategories = [];
    ...
    function renderRegionCharts(myurl) 
    {
        ...
        jQuery.getJSON(myurl, null, function (data) {
            ...
            var obj = data.regionSessionData;
            for (i = 0; i < obj.length; i++) {
                if (obj[i].region_name == 'unknown') {
                    obj[i].region_name = '<?php echo lang('t_unknow')?>'
                }
                regionSessionCategories.push(obj[i].region_name);
                regionSessionData.push(parseInt(obj[i].sessions));
            ...
            for (i = 0; i < objNewUserData.length; i++) {
                if (objNewUserData[i].region_name == 'unknown') {
                    objNewUserData[i].region_name = '<?php echo lang('t_unknow')?>'
                }
                regionNewUserCategories.push(objNewUserData[i].region_name);
                regionNewUserData.push(parseInt(objNewUserData[i].newusers));
                //regionNewUserPercentCategories.push(Math.round(objNewUserData[i].percentage*1000)/10);
            }

            changeReportType('regionsession');
    ...
    {
        if (reportType == "regionsession") {
            options.chart.renderTo = "containerRegion";
            options.series[0].data = regionSessionData;
            options.series[1].data = regionSessionData;
            options.xAxis[0].categories = regionSessionCategories;
            options.xAxis[1].categories = regionSessionPercentCategories;

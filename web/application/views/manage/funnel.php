				...
				<table cellspacing="0" class="tablesorter">
					...
					if (isset ( $result ) && ! empty ( $result )) {
						...
						<tr>
							...
							<td><a
								href="<?php echo site_url()?>/conversionrate/funnels/editFunnel/<?php echo $result['tid'][$i]?>">
				...
				<form method="post" id="form_funnel" accept-charset="UTF-8">
					...
					<div class="module_content">
						...
						<fieldset param="1" var="setp" id="fieldset1" class="fieldset">
							...
							<option value="<?php echo $event->event_sk?>"><?php echo $event->eventname?></option>
							<?php }}?>
...
$(function(){
	...
	function addstep(){
		...
		<?php if(isset($eventlist)){foreach ($eventlist->result() as $event){?>
		fieldset+='<option value="<?php echo $event->event_sk?>"><?php echo $event->eventname?></option>';
		<?php }}?>
		fieldset+='</select>';
	...
	function addfunnel(){
		...
		//loadAjax();
		$.post('<?php echo site_url()?>/conversionrate/funnels/addFunnel/'+new Date().getTime(),$('#form_funnel').serialize(),function(data,status){
			if('existsname'==data){

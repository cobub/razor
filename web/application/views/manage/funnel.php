...
if (isset($result) && ! empty($result)) {
    for ($i = 0; $i < count($result['tid']); $i ++) {
        ...
        if (empty($result['unitprice'][$i])) {
                                echo 0;
        } else {
                                echo $result['unitprice'][$i];
        }
                            ?></td>
        ...
        if (empty($result['event2_c']) || empty($result['event1_c'])) {
                  echo 0;
        } else {
					...
					<div class="module_content">
						<fieldset class="fieldset">
							<label><?php echo lang('v_rpt_re_funnelName');?></label> <input
								type="text" id="funnel_name" name="funnel_name"> <label><?php echo lang('v_rpt_re_unitprice');?></label>
							<input type="text" id="unitprice" name="unitprice">
						</fieldset>
						<fieldset param="1" var="setp" id="fieldset1" class="fieldset">
							<label><a href="javascript:rmField(1);"><img
									src="<?php echo base_url()?>assets/images/jian.png"
									style="border: 0;" /></a><?php echo lang('v_rpt_re_funnelStep');?>1</label>
...
if (isset($eventlist)) {
    foreach ($eventlist->result() as $event) {
                        ...
                        </select> <label><?php echo lang('v_rpt_re_funnelStepname');?></label>
							<input type="text" name="stepname" step="1" />
						</fieldset>

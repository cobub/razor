<section id="main" class="column" style="height:1600px">
	<article class="module width_full">
		...
		<div id="operatordetail" style="height:500px">
		</div>
	</article>
	<article class="module width_full">
		<header>
			...
			<span class="relative r"> <a
				href="<?php echo site_url().'/report/operator/exportCSV'?>"
				class="bottun4 hover"><font><?php echo lang('g_exportToCSV') ?></font></a>
			</span>
		</header>
		<table id="operatortable" class="tablesorter" cellspacing="0">
		    <thead id="operatordetailtitle"></thead>
			<tbody id="operatordetaildata"></tbody>
		</table>
	</article>
</section>
...
$(document).ready(function() {
	...
	var operatorURL  = "<?php echo site_url();?>/report/operator/getOperatorData";
	renderCharts(operatorURL);	
});

function renderCharts(myurl)
{	
	 var chart_canvas = $('#operatordetail');
	 var loading_img = $("<img src='<?php echo base_url();?>/assets/images/loader.gif'/>");
	    ...
	    jQuery.getJSON(myurl, null, function(data) {
			...
			if((j/2)>2){
				 document.getElementById("operatordetail").style.height='850px';
			}
...
function changeDetailType(type){
	...
	$("#operatordetailtitle").html(detailtitlecontent);
	$("#operatordetaildata").html(detailcontent);
}
...
function changeReportType(type)
{
	reportType=type;
	var m=0;
	var chartwidth=parseInt($("#operatordetail").css("width"))/4;
	...
	for(var key in userData){
		...
		options.chart.renderTo = "operatordetail";
		options.series[m]={};

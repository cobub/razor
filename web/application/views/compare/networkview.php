<section id="main" class="column" style="height:1500px">
	<article class="module width_full">
		<header>
			<h3 class="h3_fontstyle"><?php echo  lang('v_rpt_nw_top10')?></h3>
			<ul class="tabs2">
		...
		<div id="networkdetail" style="height:450px">
		</div>
	</article>
	<article class="module width_full">
		<header>
			<h3 class="tabs_involved"><?php echo lang('v_rpt_nw_top10') ?></h3>
			<ul class="tabs3">
			...
			<span class="relative r"> <a
				href="<?php echo site_url().'/report/network/exportCSV'?>"
				class="bottun4 hover"><font><?php echo lang('g_exportToCSV') ?></font></a>
			</span>
		</header>
		<table id="networktable" class="tablesorter" cellspacing="0">
		    <thead id="networkdetailtitle"></thead>
			<tbody id="networkdetaildata"></tbody>
		</table>
	</article>
</section>
...
$(document).ready(function() {
	...
	var networkURL  = "<?php echo site_url();?>/report/network/getNetWorkData";
	renderCharts(networkURL);
});

function renderCharts(myurl)
{	
	 var chart_canvas = $('#networkdetail');
	 var loading_img = $("<img src='<?php echo base_url();?>/assets/images/loader.gif'/>");
	    ...
	    jQuery.getJSON(myurl, null, function(data) {
			...
			if((j/2)>2){
				 document.getElementById("networkdetail").style.height='800px';
			}
...
function changeDetailType(type){
		...
		for(j=0;j<productNames.length;j++){
			...
			if(i>=obj.length){
				...
				if(obj[i].networkname==''){
					...
					detailcontent=detailcontent+"<td>"+obj[i].networkname+"</td>";
				}
	...
	$("#networkdetailtitle").html(detailtitlecontent);
	$("#networkdetaildata").html(detailcontent);
}
...
function changeReportType(type)
{
	reportType=type;
	var m=0;
	var chartwidth=parseInt($("#networkdetail").css("width"))/4;
	var chartcenter=[[chartwidth,345/2],[chartwidth*3,345/2],[chartwidth,345*1.5],[chartwidth*3,345*1.5]];
    var charttitlecontent=[{left:chartwidth-40,top:'15px'},{left:chartwidth*3-40,top:'30px'},{left:chartwidth-40,top:'365'},{left:chartwidth*3-40,top:'365'}];
	...
	for(var key in userData){
		...
		options.chart.renderTo = "networkdetail";
		options.series[m]={};

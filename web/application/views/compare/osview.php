	...
	<article class="module width_full">
		<header>
			...
			<span class="relative r"> <a
				href="<?php echo site_url().'/report/os/exportCSV'?>"
				class="bottun4 hover"><font><?php echo lang('g_exportToCSV') ?></font></a>
			</span>
		</header>
		<table id="osversiontable" class="tablesorter" cellspacing="0">
		    <thead id="osdetailtitle"></thead>
			<tbody id="osdetaildata"></tbody>
		</table>
	</article>
</section>
...
{	
	 ...
	 var loading_img = $("<img src='<?php echo base_url();?>/assets/images/loader.gif'/>");
	    ...
	    jQuery.getJSON(myurl, null, function(data) {  
			...
			if((j/2)>2){
				 document.getElementById("osversion").style.height='800px';
			}
...
function changeDetailType(type){
		...
		for(j=0;j<productNames.length;j++){
			...
			if(i>=obj.length){
				...
				if(obj[i].deviceos_name==''){
					...
					detailcontent=detailcontent+"<td>"+obj[i].deviceos_name+"</td>";
				}
				if(type=="activeuser")
	...
	$("#osdetailtitle").html(detailtitlecontent);
	$("#osdetaildata").html(detailcontent);
}
...
function changeReportType(type)
{
	...
	for(var key in userData){
		...
		options.chart.renderTo = "osversion";
		options.series[m]={};

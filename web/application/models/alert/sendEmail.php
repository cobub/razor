...
class SendEmail extends CI_Model {
	...
	function  comparevalue($date){
		...
		$emailtime = date('Y-m-d',strtotime("0 day",strtotime($date)));
		$alertlab = array();
...
// 			print_r($trendret);
			...
			if($label=="t_newUser"){
				...
				$msg = $this->emailText($productname,lang('t_newUsers'), $ret[0]['newusers'], $result[0]['newusers'], $condition,$url,$emailtime);
			...
			if($label=="t_activeUser"){
				...
				$msg = $this->emailText($productname,lang('t_activeUser'), $ret[0]['startusers'], $result[0]['startusers'], $condition,$url,$emailtime);
			...
			if($label=="t_sessions"){
				...
				$msg = $this->emailText($productname,lang('t_sessions'), $ret[0]['sessions'], $result[0]['sessions'], $condition,$url,$emailtime);
			...
			if($label=="t_accumulatedUsers"){
				...
				$msg = $this->emailText($productname,lang('t_accumulatedUsers'), $ret[0]['allusers'], $result[0]['allusers'], $condition,$url,$emailtime);
			...
			if($label=="t_averageUsageDuration"){
				...
				$msg = $this->emailText($productname,lang('t_averageUsageDuration'), $ret[0]['usingtime'], $result[0]['usingtime'], $condition,$url,$emailtime);
	...
	function emailText($product,$biaobiao,$shiji,$qushi,$fanwei,$url,$time){
		$text = lang('emailtext');
		$text = sprintf($text,$product,$time,$biaobiao,$qushi,$shiji,$fanwei,$url);
		return $text;
	}

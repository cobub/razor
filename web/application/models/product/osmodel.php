...
class osmodel extends CI_Model{
	...
	//获取所有用户系统比例
	function getTotalUserPercentByOS($productId)
	{
		$dwdb = $this->load->database('dw',TRUE);
		$sql = "select o.deviceos_name,
		count(distinct f.deviceidentifier) / (select count(distinct ff.deviceidentifier,o.deviceos_name) percent 
		from  ".$dwdb->dbprefix('fact_clientdata')." ff,".$dwdb->dbprefix('dim_product')." pp,".$dwdb->dbprefix('dim_deviceos')." oo 
		where ff.product_sk = pp.product_sk and ff.deviceos_sk = oo.deviceos_sk and pp.product_id = $productId and pp.product_active=1 and pp.channel_active=1 and pp.version_active=1) percentage 
		from   ".$dwdb->dbprefix('fact_clientdata')." f,".$dwdb->dbprefix('dim_product')." p,".$dwdb->dbprefix('dim_deviceos')." o 
		where  f.product_sk = p.product_sk and f.deviceos_sk = o.deviceos_sk 
